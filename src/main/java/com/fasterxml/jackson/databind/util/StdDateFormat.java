...
public class StdDateFormat extends DateFormat
{
    ...
    protected Date parseAsISO8601(String dateStr, final ParsePosition pos) {
        ...
        if (len <= 10 && Character.isDigit(c)) {
            ...
            len = dateStr.length();
            final int timeLen = len - dateStr.lastIndexOf(84) - 6;
            if (timeLen < 12) {
                final int offset = len - 5;
                final StringBuilder sb2 = new StringBuilder(dateStr);
                switch (timeLen) {
                    case 11: {
                        sb2.insert(offset, '0');
                        break;
                    ...
                    case 10: {
                        sb2.insert(offset, "00");
                        break;
                    }
                    case 9: {
                        sb2.insert(offset, "000");
                        break;
                    }
                    case 8: {
                        sb2.insert(offset, ".000");
            ...
            final StringBuilder sb = new StringBuilder(dateStr);
            final int timeLen2 = len - dateStr.lastIndexOf(84) - 1;
            if (timeLen2 < 12) {
                switch (timeLen2) {
                    case 11: {
                        sb.append('0');
