...
public final class MinimumValidator extends NumericValidator
{
    ...
    protected void validateLong(final ProcessingReport report, final MessageBundle bundle, final FullData data) throws ProcessingException {
        ...
        if (instanceValue < longValue) {
            report.error(this.newMsg(data, bundle, "err.common.minimum.tooSmall").putArgument(this.keyword, this.number).putArgument("found", instance));
            return;
        }
        if (!this.exclusive) {
            return;
        }
        report.error(this.newMsg(data, bundle, "err.common.minimum.notExclusive").putArgument(this.keyword, this.number).put("exclusiveMinimum", (JsonNode)BooleanNode.TRUE));
    }
