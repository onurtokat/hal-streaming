...
public final class AllOfValidator extends SchemaArrayValidator
{
    ...
    public void validate(final Processor<FullData, FullData> processor, final ProcessingReport report, final MessageBundle bundle, final FullData data) throws ProcessingException {
        ...
        final ObjectNode fullReport = AllOfValidator.FACTORY.objectNode();
        int nrSuccess = 0;
        ...
        if (nrSuccess != size) {
            report.error(this.newMsg(data, bundle, "err.draftv4.allOf.fail").putArgument("matched", nrSuccess).putArgument("nrSchemas", size).put("reports", (JsonNode)fullReport));
        }
    }
}
