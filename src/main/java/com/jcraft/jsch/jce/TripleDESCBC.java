...
public class TripleDESCBC implements Cipher
{
    ...
    public void init(final int n, byte[] array, byte[] array2) throws Exception {
        ...
        try {
            (this.cipher = javax.crypto.Cipher.getInstance("DESede/CBC/" + s)).init((n == 0) ? 1 : 2, SecretKeyFactory.getInstance("DESede").generateSecret(new DESedeKeySpec(array)), new IvParameterSpec(array2));
        }
        catch (Exception ex) {
            this.cipher = null;
            throw ex;
