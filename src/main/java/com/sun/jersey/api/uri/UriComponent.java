...
public class UriComponent
{
    ...
    private static String _encode(final String s, final Type t, final boolean template, final boolean contextualEncode) {
        ...
        for (int i = 0; i < s.length(); ++i) {
            ...
            else if (template && (c == '{' || c == '}')) {
                if (sb != null) {
                    sb.append(c);
                }
            }
            else if (contextualEncode && c == '%' && i + 2 < s.length() && isHexCharacter(s.charAt(i + 1)) && isHexCharacter(s.charAt(i + 2))) {
                if (sb != null) {
    ...
    private static void appendUTF8EncodedCharacter(final StringBuilder sb, final char c) {
        final ByteBuffer bb = UriComponent.UTF_8_CHARSET.encode("" + c);
    ...
    private static void decodeQueryParam(final MultivaluedMap<String, String> params, final String param, final boolean decode) {
        try {
            final int equals = param.indexOf(61);
            if (equals > 0) {
                params.add(URLDecoder.decode(param.substring(0, equals), "UTF-8"), decode ? URLDecoder.decode(param.substring(equals + 1), "UTF-8") : param.substring(equals + 1));
            }
            else if (equals != 0) {
                if (param.length() > 0) {
