...
public class WebComponent implements ContainerListener
{
    ...
    private boolean useSetStatusOn404;
    ...
    public int service(final URI baseUri, final URI requestUri, final HttpServletRequest request, final HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            final Writer w = new Writer(this.useSetStatusOn404, response);
    ...
    private ResourceConfig createResourceConfig(final WebConfig webConfig, final Map<String, Object> props) throws ServletException {
        ...
        if (resourceConfigClassName == null) {
            ...
            try {
                final Class<?> resourceConfigClass = (Class<?>)ReflectionHelper.classForNameWithException(resourceConfigClassName);
    ...
    private void filterFormParameters(final HttpServletRequest hsr, final ContainerRequest cr) throws IOException {
        if (cr.getMethod().equals("POST") && MediaTypes.typeEquals(MediaType.APPLICATION_FORM_URLENCODED_TYPE, cr.getMediaType()) && !this.isEntityPresent(cr)) {
            final Form f = new Form();
            final Enumeration e = hsr.getParameterNames();
            while (e.hasMoreElements()) {
                final String name = e.nextElement();
                final String[] values = hsr.getParameterValues(name);
    ...
    private InBoundHeaders getHeaders(final HttpServletRequest request) {
        ...
        final Enumeration<String> names = (Enumeration<String>)request.getHeaderNames();
        while (names.hasMoreElements()) {
            final String name = names.nextElement();
            final List<String> valueList = new LinkedList<String>();
            final Enumeration<String> values = (Enumeration<String>)request.getHeaders(name);
    ...
    private static final class Writer extends OutputStream implements ContainerResponseWriter
    {
        ...
        Writer(final boolean useSetStatusOn404, final HttpServletResponse response) {
        ...
        public void finish() throws IOException {
            ...
            if (this.cResponse.getStatus() >= 400) {
                if (this.useSetStatusOn404 && this.cResponse.getStatus() == 404) {
                    ...
                    final String reason = this.cResponse.getStatusType().getReasonPhrase();
