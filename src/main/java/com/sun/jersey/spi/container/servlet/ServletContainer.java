    ...
    public void service(final HttpServletRequest request, final HttpServletResponse response) throws ServletException, IOException {
        ...
        if (checkPathInfo && checkServletPath && !request.getRequestURI().endsWith("/")) {
            if (this.webComponent.getResourceConfig().getFeature("com.sun.jersey.config.feature.Redirect")) {
                final URI l = UriBuilder.fromUri(request.getRequestURL().toString()).path("/").replaceQuery(request.getQueryString()).build(new Object[0]);
        ...
        final UriBuilder absoluteUriBuilder = UriBuilder.fromUri(requestURL.toString());
        final String decodedBasePath = (pathInfo != null) ? (request.getContextPath() + servletPath + "/") : (request.getContextPath() + "/");
