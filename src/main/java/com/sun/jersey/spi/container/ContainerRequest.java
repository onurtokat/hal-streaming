...
public class ContainerRequest implements HttpRequestContext
{
    ...
    private String getEncodedPath() {
        ...
        return this.encodedPath = this.getRequestUri().getRawPath().substring(this.getBaseUri().getRawPath().length());
    }
    ...
    public <T> T getEntity(final Class<T> type, final Type genericType, final Annotation[] as) {
        ...
        if (bw == null) {
            ...
            message = message + "The registered message body readers compatible with the MIME media type are:\n" + this.getMessageBodyWorkers().readersToString(m);
            ContainerRequest.LOGGER.severe(message);
