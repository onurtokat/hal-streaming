...
public class ContainerResponse implements HttpResponseContext
{
    ...
    public void write() throws IOException {
        ...
        final MessageBodyWriter p = this.getMessageBodyWorkers().getMessageBodyWriter(this.entity.getClass(), this.entityType, this.annotations, contentType);
        if (p != null) {
            final long size = p.getSize(this.entity, this.entity.getClass(), this.entityType, this.annotations, contentType);
            if (this.request.getMethod().equals("HEAD")) {
                ...
                p.writeTo(this.entity, this.entity.getClass(), this.entityType, this.annotations, contentType, this.getHttpHeaders(), this.out);
                if (!this.isCommitted) {
                    this.isCommitted = true;
                    this.responseWriter.writeStatusAndHeaders(-1L, this);
                }
        ...
        ContainerResponse.LOGGER.severe(message);
        final Map<MediaType, List<MessageBodyWriter>> m = this.getMessageBodyWorkers().getWriters(contentType);
        ContainerResponse.LOGGER.severe("The registered message body writers compatible with the MIME media type are:\n" + this.getMessageBodyWorkers().writersToString(m));
        if (this.request.getMethod().equals("HEAD")) {
            this.isCommitted = true;
            this.responseWriter.writeStatusAndHeaders(-1L, this);
            this.responseWriter.finish();
