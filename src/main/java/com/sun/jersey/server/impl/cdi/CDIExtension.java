...
public class CDIExtension implements Extension
{
    ...
    public CDIExtension() {
        ...
        this.injectAnnotationLiteral = (Inject)new InjectAnnotationLiteral();
    ...
    void afterBeanDiscovery(@Observes final AfterBeanDiscovery event) {
        ...
        for (final Map.Entry<Class<? extends Annotation>, Set<DiscoveredParameter>> entry : this.discoveredParameterMap.entrySet()) {
            final Class<? extends Annotation> qualifier = entry.getKey();
            for (final DiscoveredParameter parameter : entry.getValue()) {
    ...
    class PredefinedBean<T> extends AbstractBean<T>
    {
        ...
        public T create(final CreationalContext<T> creationalContext) {
            final Injectable<T> injectable = (Injectable<T>)CDIExtension.this.webApplication.getServerInjectableProviderFactory().getInjectable(this.qualifier.annotationType(), null, this.qualifier, this.getBeanClass(), ComponentScope.Singleton);
            if (injectable == null) {
    ...
    class ParameterBean<T> extends AbstractBean<T> implements InitializedLater
    {
        ...
        public void later() {
            ...
            this.processed = true;
            final boolean registered = CDIExtension.this.webApplication.getServerInjectableProviderFactory().isParameterTypeRegistered(this.parameter);
            if (!registered) {
                Errors.error("Parameter type not registered " + this.discoveredParameter);
            }
            this.injectable = (Injectable<T>)CDIExtension.this.webApplication.getServerInjectableProviderFactory().getInjectable(this.parameter, ComponentScope.PerRequest);
            if (this.injectable == null) {
        ...
        public T create(final CreationalContext<T> creationalContext) {
            ...
            try {
                ...
                if (this.injectable instanceof AbstractHttpContextInjectable) {
                    return ((AbstractHttpContextInjectable)this.injectable).getValue(CDIExtension.this.webApplication.getThreadLocalHttpContext());
                }
                throw e;
            }
        }
