...
public class WadlBuilder
{
    ...
    public ApplicationDescription generate(final Set<AbstractResource> resources) {
    ...
    public Application generate(final ApplicationDescription description, final AbstractResource resource) {
        final Application wadlApplication = this._wadlGenerator.createApplication();
        final Resources wadlResources = this._wadlGenerator.createResources();
    ...
    public Application generate(final ApplicationDescription description, final AbstractResource resource, final String path) {
        final Application wadlApplication = this._wadlGenerator.createApplication();
        final Resources wadlResources = this._wadlGenerator.createResources();
    ...
    private Method generateMethod(final AbstractResource r, final Map<String, Param> wadlResourceParams, final AbstractResourceMethod m) {
        ...
        final Response wadlResponse = this.generateResponse(r, m);
    ...
    private Request generateRequest(final AbstractResource r, final AbstractResourceMethod m, final Map<String, Param> wadlResourceParams) {
        ...
        for (final Parameter p : m.getParameters()) {
            if (p.getSource() == Parameter.Source.ENTITY) {
                ...
                if (wadlParam2.getStyle() == ParamStyle.TEMPLATE) {
    ...
    private Response generateResponse(final AbstractResource r, final AbstractResourceMethod m) {
        if (m.getMethod().getReturnType() == Void.TYPE) {
            return null;
        }
        final Response wadlResponse = this._wadlGenerator.createResponse(r, m);
        return wadlResponse;
    }
}
