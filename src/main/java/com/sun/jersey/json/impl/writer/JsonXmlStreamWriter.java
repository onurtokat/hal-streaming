...
public class JsonXmlStreamWriter implements XMLStreamWriter
{
    ...
    private JsonXmlStreamWriter(final Writer writer, final JSONConfiguration config) {
    ...
    public static XMLStreamWriter createWriter(final Writer writer, final JSONConfiguration config) {
        final Collection<String> attrsAsElems = config.getAttributeAsElements();
        if (attrsAsElems != null && !attrsAsElems.isEmpty()) {
            return new A2EXmlStreamWriterProxy(new JsonXmlStreamWriter(writer, config), attrsAsElems);
        }
